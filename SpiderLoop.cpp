...
subloopNextPriority:
	...
	if ( cr->m_maxCrawlRounds > 0 &&
		...
		logTrace( g_conf.m_logTraceSpider, "Loop, crawl round max reached" );
		goto subloop;
	}
	...
	if ( cr->m_maxToCrawl > 0 &&
		...
		logTrace( g_conf.m_logTraceSpider, "Loop, max pages to crawl reached" );
		goto subloop;
	}
	...
	if ( cr->m_maxToProcess > 0 &&
		...
		logTrace( g_conf.m_logTraceSpider, "Loop, max pages to process reached" );
		goto subloop;
	}
