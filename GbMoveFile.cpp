...
int moveFile(const char *src, const char *dst)
{
	...
	if(errno!=EXDEV) {
		...
		//An error we don't know how to handle
		log(LOG_ERROR,"moveFile:rename(%s,%s) failed with errno=%d (%s)", src,dst,errno,strerror(errno));
		errno = saved_errno;
		return -1;
	}

	//rename() didn't work because the source and destination are on different
	//devices (filesystems). Try to do copy+delete
	...
	if(unlink(src)!=0) {
		log(LOG_ERROR,"moveFile:unlink(%s) failed with errno=%d (%s)", src,errno,strerror(errno));
		//Error is otherwise ignored. If unlinking the source file failed then we don't really know
		//what to do. Ok, if the source file system is read-only then we should have failed before
		//starting to copy, but rw/ro state and file access rights can chagne during the copy, so.. meh.
	}
