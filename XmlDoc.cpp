...
Query *XmlDoc::getQuery() {
	...
	// return NULL with g_errno set on error
	if (!m_query.set2(m_req->ptr_qbuf, (lang_t)m_req->m_langId, m_req->m_wiktionaryWordVariations, m_req->m_languageSpecificWordVariations, m_req->m_useQueryStopWords, m_req->m_allowHighFrequencyTermCache)) {
		if(!g_errno)
