...
Additional, testing if a codepoint can be in a word/identifier (a-z, 0-9, æ, greek letters, ...) is the most used lookup so we make a specialized map for that: full map, 1-byte boolean per entry. It may be worth squeezing down to 1 bit per codepoint.
